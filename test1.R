...
for (i in seq_along(model)) {
  ...
  nc_data <- nc_open(s_temp_files[i]) # Open surftemp file
  st <- ncvar_get(nc_data, "surftemp")
  nc_close(nc_data)
  
  nc_data <- nc_open(strat_files[i]) # Open strat file
...
kivu_pi_LER$strat <- ifelse(kivu_pi_LER$strat == 0, NA, kivu_pi_LER$strat)

# Find longest contiguous non-NA values for each year
longest_stretch_LER <- kivu_pi_LER |>
  mutate(year = year(dates)) |>
  group_by(year, model, gcm) |>
  group_modify(~ {
...
Kivu_LER <- pmap_dfr(combinations, get_yearly_averages, data = longer_strat_LER) # Applying the function to each model and combining the results
